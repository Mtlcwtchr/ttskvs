...
--
-- Теперь размер объявляется НАМЕРЕНИЕМ, а пиксели считает этот модуль:
--
...
-- Единственное место с «естественными» размерами листьев. Текст в песочнице
...
-- занимает всю внутреннюю область. Так рисуются подложки и оверлеи.
--
-- Тонкость, из-за которой шапка раньше и разъезжалась: TTS считает
